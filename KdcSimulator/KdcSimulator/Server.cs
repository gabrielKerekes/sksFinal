        ...
        private void ReceiveMessagesLoop(TcpClient client)
        {
            while (true)
            {
                try
                {
                    ...
                    var messageSequence = new byte[] { 0x17, 0x2B, 0x10, 0x29, 0x13, 0x0F, 0x31, 0x31, 0x31, 0x2E, 0x31, 0x31, 0x31, 0x2E, 0x31, 0x31, 0x31, 0x2E, 0x31, 0x31, 0x31, 0x02, 0x01, 0x05, 0x13, 0x0e, 0x41, 0x6e, 0x79, 0x62, 0x6f, 0x64, 0x79, 0x20, 0x74, 0x68, 0x65, 0x72, 0x65, 0x3f, 0x04, 0x00, 0x13, 0x01, 65, };
                    var message = $"-----BEGIN INITIAL MESSAGE 2-----\n{Convert.ToBase64String(messageSequence)}\n------END INITIAL MESSAGE 2";
                    client.GetStream().Write(new byte[] {0x00, 0x00, 0x00, 0x7A}.Concat(Encoding.ASCII.GetBytes(message)).ToArray(), 0, message.Length);
